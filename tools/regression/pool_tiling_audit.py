...
# the quad's size and its texture must come from the SAME n, or the tiling is off by a scale factor
check(re.search(r"var n := _pool_cells\(d\)\s*\n\s*gm\.size = Vector2\(n, n\)", src) is not None,
      "quad size and texture come from the same `n`")
check(re.search(r"_fx_material\(_pool_texture\(n\), true\)", src) is not None,
      "the pool asks _fx_material for NEAREST")
